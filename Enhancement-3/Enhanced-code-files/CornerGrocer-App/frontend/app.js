 ...
 *               Upgraded API connections to dynamically switch between local testing and 
 *               production domains. Rerouted search queries directly to the server's 
 *               optimized B-Tree search endpoint to maximize algorithmic data scaling.
 *               Integrated client-side try/catch blocks to protect the user 
 *               interface from crashing during network dropouts and ensure all database 
...
const CONFIG = {
    // DYNAMIC DOMAIN DETECTOR: Automatically handles local localhost paths vs live Render host urls
...
document.getElementById('search-btn').addEventListener('click', async () => {
    ...
    try {
        // ENHANCEMENT 3 COMPLETED UPGRADE: HIGH-PERFORMANCE DISK RETRIEVAL
        // To maximize efficiency, we have successfully rerouted this search fetch call away 
        // from downloading the entire dataset array. It points straight to our optimized backend path: 
        // `/api/items/search?name=${query}`. Now that MongoDB is active, this query targets our 
        // indexed collection using a B-Tree structure, running lookups at an ultra-fast O(log n) complexity!
        const response = await fetch(`${CONFIG.API_URL}/items/search?name=${encodeURIComponent(query)}`);
        
        // Clear graph view out of the way for search results
        graphView.classList.add('hidden');

        if (response.status === 404) {
            // Handle database missing responses cleanly
            renderTableMessage(
        ...
        // Put the single database result into an array so our table renderer can read it cleanly
        let itemsToDisplay = [foundItem];

        // Apply our date filter constraint on the individual item response if necessary
        if (selectedDate !== 'all' && foundItem.date !== selectedDate) {
            itemsToDisplay = [];
        }

        // Show the results table if the query constraints match, otherwise display a missing alert row
        if (itemsToDisplay.length > 0) {
            renderTable(itemsToDisplay);
            tableView.classList.remove('hidden');
        } else {
            renderTableMessage(
                `🔍 No results for "<strong>${rawQuery}</strong>" on this date`, 
                "Try changing your calendar filter to 'All Dates' to locate this transaction record.",
                CONFIG.DEFAULT_COLOR
            );
        }
    } catch (error) {
 ...
 * --- ENHANCEMENT 3 PERSISTENT UPGRADE ---
 * Data fields passed into this rendering structure are now pulled directly from our live, 
 * cloud-managed MongoDB tracking log, maintaining responsive client-side visual display speeds.
 */
